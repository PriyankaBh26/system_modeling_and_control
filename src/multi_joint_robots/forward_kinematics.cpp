...
void ForwardKinematics::TfMatrixInBaseFrame() {
    ...
    for (int i(num_joints-1); i>0; i--) {
        tf_se = ForwardKinematics::ExponentialMatrix(screw_axes_s.col(i), q(i), joint_type(i)) * tf_se;
    }
};
...
MatrixXd ForwardKinematics::ExponentialMatrix(VectorXd screw_axis, double q_i, std::string joint_type_i) {
    ...
    if (joint_type_i == "R") {
        ...
        exp_q(3,1,0,3) = (MatrixXd::Identity(3,3) + (1 - cos(q_i)) * W + (q_i - sin(q_i)) * W * W) * v;
    
    } else if (joint_type_i == "P") {
        // prismatic joint
        exp_q(3,3,0,0) = MatrixXd::Identity(3,3);
